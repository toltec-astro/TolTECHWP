...
class UDPPackagingConsumerThread(threading.Thread):
    ...
    def send_message(self, message, ip='192.168.110.207', port=64013):
        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM) # UDP
        sock.bind(('192.168.110.221', port + 1000))
    ...
    def run(self):
        ...
        while not self.shutdown_flag or not self.quad_queue.empty():
            ...
            remaining = self.quad_queue.qsize()
            ...
            #print(r_sensors_queue, sensor_export)
            ...
            # make the packet
            packet = list(quad_export[0:166*3]) + [0]*14 + list(quad_export[166*3:332*3]) + [0]*14 + list(quad_export[332*3:498*3]) + [0]*14 + list(quad_export[498*3:500*3]) + list(zeroopt_export) + list(pps_export) + list(sensor_export)

            packet += [0]*(2048-len(packet))
            #[SRC_CTIME0, SRC_CTIME1, SRC_PPSCOUNT, SRC_CLOCKCOUNT, SRC_PPSTIME, SRC_PACKETCOUNT]
            latest_pps = pps_export[0:3] 
            packet[1024*2-6+5] = socket.htonl(self.packets_sent)
            packet[1024*2-6+2] = socket.htonl(int(pps_export[1]))#socket.htonl(latest_pps)
            packet[1024*2-6+3] = socket.htonl(int(pps_export[0]))
            packet[1024*2-6+4] = socket.htonl(int(pps_export[2]))
            bin_packet = [struct.pack("=I", value) for value in packet]
            binarypackage = b"".join(bin_packet)
            ...
            # packet sending loop
            try:
                self.send_message(binarypackage)
                msg = f'{int(time.time())} Packet Size Sent: {packet_size} Packets Sent: {self.packets_sent} {packet[2]} {packet[500+3*14+3+3+3-1]} {packet[512*3+1]} {remaining}'
                print(msg)
                self.packets_sent += 1
            ...
            # local save
            try:
                self.send_message(binarypackage, ip='127.0.0.1', port=6969)
            except OSError as error:
                pass
