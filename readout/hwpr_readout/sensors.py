    ...
    def run(self):
        ...
        while not self.shutdown_flag:
            ...
            for slot_num, volt in zip(slotpylist, avg_voltage_arr):
                ...
                self.queue.put((slot_num, volt * 1000, timestamp))
                #self.sensorlog.info(f'{slot_num}\t{volt:08.5f}\t{timestamp}')
            #print(timestamp, avg_voltage_arr)            
            temp_volt_out = voltage[0]
            humi_volt_out = voltage[1]
            psu_temp_volt_out = voltage[14]
            cacp_volt_out = voltage[10]
            

            
            cac_pressure = cacp_volt_out * 20
            temp = (temp_volt_out / 0.01) # C degrees
            psu_temp = (psu_temp_volt_out / 0.01)
            sensor_rh = ((humi_volt_out / 5) - 0.16 ) /  0.0062
            true_rh = (sensor_rh) / (1.0546 - (0.00216 * temp))
            if self.debug == True:
                print(avg_voltage_arr)
                print(f'48V Temperature: {psu_temp:0.1f} C ({((psu_temp * 9/5) + 32):0.1f} F)')
                print(f'Temperature: {temp:0.1f} C ({((temp * 9/5) + 32):0.1f} F) \t Humidity: {true_rh:0.1f} % \t CAC Pressure: {cac_pressure:0.1f} psi')
...
if __name__ == '__main__':
    ...
    test_SensorThread = SensorProducerThread(test_S826Board, config, queue.Queue(maxsize=-1), sdqueue, debug=True)
