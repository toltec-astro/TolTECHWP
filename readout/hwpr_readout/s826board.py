...
class S826Board(object):
    ...
    def _configure_pps(self):
        ...
        S826_BITWRITE = 0
        S826_CM_PX_IXRISE = (1 << 15)
        S826_CM_XS_EXTNORMAL = 0
        ...
        #pps_flags= self.s826_dll.S826_CounterPreloadWrite(
        #     self.board_id, countpps, 0, 0
        #)
        #errhandle(pps_flags, name=f'S826_CounterPreloadWrite')

        #pps_flags = self.s827_dll.S826_CounterExtInRoutingWrite(
        #    self.board_id, countpps, 48 + countpps
        #)
        #errhandle(pps_flags, name=f'S826_CounterExtInRoutingWrite')

        # implementation of any filter appears to negatively affect the signal
        # the pps shouldn't be filtered out by a 60 ns filter
        # it appears that 60 ns (F=3) successfully filters out the other stuff
